...
export default function SearchFilters({ filters, onFilterChange }) {
  ...
  return (
    <div className="border p-3 mb-8" style={{ background: '#ece9e2', borderColor: 'rgba(26,39,68,0.12)' }}>
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: 'rgba(26,39,68,0.3)' }} />
          <Input placeholder="Search creators or neighborhoods" value={filters.search || ''} onChange={e => update('search', e.target.value)}
            className="pl-9 h-10 rounded-none border-0 bg-transparent text-[12px]" style={{ color: '#1a2744' }} />
        </div>
        <Select value={filters.eventType || 'all'} onValueChange={v => update('eventType', v)}>
          <SelectTrigger className="w-full sm:w-40 h-10 rounded-none border-0 bg-transparent text-[12px]"><SelectValue /></SelectTrigger>
          <SelectContent>{EVENT_TYPES.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent>
        </Select>
        <Select value={filters.style || 'all'} onValueChange={v => update('style', v)}>
          <SelectTrigger className="w-full sm:w-36 h-10 rounded-none border-0 bg-transparent text-[12px]"><SelectValue /></SelectTrigger>
